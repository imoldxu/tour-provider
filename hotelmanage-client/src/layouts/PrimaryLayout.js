...
class PrimaryLayout extends PureComponent {
  ...
  render() {
    ...
    const agency = store.get('agency') || {}
    //const user = store.get('user') || {}
    //屏蔽权限
    //const permissions = store.get('permissions') || {}
    //const routeList = store.get('routeList') || []
    const routeList = store.get('routeList') || app.routeList //可以从store中读取sider列表（可以来源于后台返回存储下来的），若没有，则从app配置读取
    const { isMobile } = this.state
    ...
    // 处理用户是否有权限访问该页面，由页面路由id与权限列表匹配来判断
